...
function syncLiveCard(device){
  ...
  if(!element)return;
  const serverSession=Math.max(0,Number(device.session_remaining_seconds)||0),
        serverBreak=Math.max(0,Number(device.break_remaining_seconds)||0);
  element.dataset.clockSession=String(serverSession);
  element.dataset.clockBreak=String(serverBreak);
...
const clockCard=card;card=function(device){const element=clockCard(device),serverSession=Math.max(0,Number(device.session_remaining_seconds)||0),serverBreak=Math.max(0,Number(device.break_remaining_seconds)||0);element.dataset.clockSession=String(serverSession);element.dataset.clockBreak=String(serverBreak);element.dataset.clockBlocked=String(!!device.blocked);element.dataset.clockManual=String(!!device.manual_blocked);element.dataset.clockOffline=String(!device.ip);element.dataset.clockNight=String(hasNight(device));return element};
const clockUpdate=updateTimers;updateTimers=function(){clockUpdate();if(document.visibilityState==='hidden')return;document.querySelectorAll('.card[data-id]').forEach(card=>{const session=Math.max(0,Number(card.dataset.clockSession)||0),breakLeft=Math.max(0,Number(card.dataset.clockBreak)||0),frozen=card.dataset.clockBlocked==='true'||card.dataset.clockManual==='true'||card.dataset.clockOffline==='true'||card.dataset.clockNight==='true',sessionLine=$('.session-left',card),restLine=$('.rest-left',card),active=card.dataset.breakActive==='true';if(sessionLine&&!frozen&&!active&&card.dataset.clockNight!=='true')sessionLine.textContent=`До отдыха осталось: ${duration(session)}`;if(restLine&&active)restLine.textContent=`До окончания отдыха: ${duration(breakLeft)}`})};
refresh();
