...
static void tick(void){static time_t last_dhcp_sync=0;time_t now_sync=monotonic_now();if(now_sync-last_dhcp_sync>=2){sync_dhcp_macs();last_dhcp_sync=now_sync;}
    ...
    for(int i=0;i<(int)json_object_array_length(a)&&i<128;i++){
        ...
        if(have){
            if(counters_seen[i]){unsigned long long di=inb[i]>=previous_inbound[i]?inb[i]-previous_inbound[i]:inb[i]; unsigned long long do_=outb[i]>=previous_outbound[i]?outb[i]-previous_outbound[i]:outb[i]; if(!before&&di+do_>0){active=1;last_active[i]=mono;}}
