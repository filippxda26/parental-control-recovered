...
static int counters_seen[128], blocked_cache[128]; static time_t last_active[128];
...
static void tick(void){static time_t last_dhcp_sync=0;time_t now_sync=time(NULL);if(now_sync-last_dhcp_sync>=2){sync_dhcp_macs();last_dhcp_sync=now_sync;}
    ...
    for(int i=0;i<(int)json_object_array_length(a)&&i<128;i++){
        ...
        int before=is_blocked(d,i),cycle=bval(d,"session_limit_enabled",0)&&bval(d,"break_enabled",0),active=0;
        if(!cycle&&(session_seconds[i]||break_active[i]||brk_until[i])){session_seconds[i]=0;break_active[i]=0;brk_until[i]=0;changed=1;if(before)rules_changed=1;}
        if(have){
...
static void handle(int c){char b[65536];ssize_t n=read_http_request(c,b,sizeof b);if(n<=0)return;if(n==-2){json_object*j=okmsg("request too large");json_object_object_add(j,"success",json_object_new_boolean(0));response(c,413,j);json_object_put(j);return;}b[n]=0;char method[8],path[512];if(sscanf(b,"%7s %511s",method,path)!=2)return;char *body=strstr(b,"\r\n\r\n");body=body?body+4:(char*)"";json_object*j=NULL;int code=200;
