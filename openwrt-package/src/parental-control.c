...
static int read_nft_counters(unsigned long long *inb,unsigned long long *outb){
    ...
    while(fgets(line,sizeof line,f)){
        ...
        c+=13; char *e=strchr(c,'\"'); if(!e)continue; *e=0; char *dir=strrchr(c,':'); if(!dir)continue; *dir++=0;
...
static int is_blocked(json_object*d,int i){int lim=ival(d,"daily_limit_minutes",0)*60+(int)bonus_minutes[i]*60;return manual[i]||(!temporary_unblock[i]&&(night(d)||(break_active[i]&&time(NULL)<brk_until[i])||(bval(d,"daily_limit_enabled",0)&&lim>0&&used_seconds[i]>=lim)));}
...
static void save(void){json_object_to_file_ext(DEVICES,root,JSON_C_TO_STRING_PRETTY);}
...
static int sync_dhcp_macs(void){FILE*f=fopen("/tmp/dhcp.leases","r");if(!f)return 0;int changed=0;char line[768],mac[32],ip[64],host[128];while(fgets(line,sizeof line,f)){long expiry=0;mac[0]=ip[0]=host[0]=0;if(sscanf(line,"%ld %31s %63s %127s",&expiry,mac,ip,host)<4)continue;for(int i=0;i<(int)json_object_array_length(arr());i++){json_object*d=json_object_array_get_idx(arr(),i);const char*old=sval(d,"mac","");if(device_has_hostname(d,host)&&strcasecmp(old,mac)&&!known_mac(mac)){char oldcopy[32];snprintf(oldcopy,sizeof oldcopy,"%s",old);json_object_object_add(d,"mac",json_object_new_string(mac));fprintf(stderr,"parental-control: DHCP update applied device_id=%s old_mac=%s new_mac=%s ip=%s nft_result=success\n",sval(d,"id",""),oldcopy,mac,ip);changed=1;}}}fclose(f);if(changed){save();nft_apply();}return changed;}
...
static const char *validate_device(json_object*d,int editing){json_object*v,*hs;if(!d||!json_object_is_type(d,json_type_object))return "invalid device settings";const char*name=sval(d,"name","");const char*mac=sval(d,"mac","");if(!*name||!mac_valid(mac)||!json_object_object_get_ex(d,"hostnames",&hs)||!json_object_is_type(hs,json_type_array)||json_object_array_length(hs)<1)return "id, name and hostname are required";for(int i=0;i<(int)json_object_array_length(hs);i++){const char*h=json_object_get_string(json_object_array_get_idx(hs,i));if(!hostname_valid(h))return "invalid hostname";for(int k=0;k<i;k++)if(!strcasecmp(h,json_object_get_string(json_object_array_get_idx(hs,k))))return "duplicate hostname";if(!editing&&known_hostname(h))return "duplicate hostname";}if(ival(d,"daily_limit_minutes",0)<0||ival(d,"session_limit_minutes",0)<0||ival(d,"break_minutes",0)<0)return "invalid device limits";if(bval(d,"night_enabled",0)&&(!json_object_object_get_ex(d,"night_start",&v)||!*json_object_get_string(v)||!json_object_object_get_ex(d,"night_end",&v)||!*json_object_get_string(v)))return "night start and end are required";return NULL;}
