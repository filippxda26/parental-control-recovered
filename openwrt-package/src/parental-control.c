...
static int make_unique_device_id(const char*mac,char out[64]){char base[64];make_device_id(mac,base);if(findid(base)<0){snprintf(out,64,"%s",base);return 0;}for(int n=2;n<1000;n++){snprintf(out,64,"%s-%d",base,n);if(findid(out)<0)return 0;}return-1;}
