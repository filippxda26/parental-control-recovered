...
static int mac_valid(const char*m){if(!m||strlen(m)!=17)return 0;for(int i=0;i<17;i++){if(i%3==2){if(m[i]!=':')return 0;}else if(!isxdigit((unsigned char)m[i]))return 0;}return 1;}
