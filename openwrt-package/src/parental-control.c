...
static void dhcp(json_object*d,char *ip,size_t ni){ip[0]=0;DhcpLease leases[256];int n=read_leases(leases,256);const char*want=sval(d,"mac","");for(int i=0;i<n;i++)if(!strcasecmp(leases[i].mac,want)){snprintf(ip,ni,"%s",leases[i].ip);return;}}
