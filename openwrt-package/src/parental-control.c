...
static void tick(void){static time_t last_dhcp_sync=0;time_t now_sync=monotonic_now();if(now_sync-last_dhcp_sync>=2){sync_dhcp_macs();last_dhcp_sync=now_sync;}
    ...
    for(int i=0;i<(int)json_object_array_length(a)&&i<128;i++){
        ...
        int after=is_blocked(d,i),wl=whitelist_active(d); if(after!=before||after!=blocked_cache[i]||wl!=whitelist_cache[i])rules_changed=1;json_object*wle=NULL;if(after&&wl&&json_object_object_get_ex(d,"whitelist_entries",&wle)&&json_object_is_type(wle,json_type_array)&&json_object_array_length(wle)>0)whitelist_refresh_needed=1;
    }
...
static void nft_allow_whitelist_address(FILE*f,const char*m,const char*id,int family,const void*addr){char ip[INET6_ADDRSTRLEN];if(!inet_ntop(family,addr,ip,sizeof ip))return;if(family==AF_INET){fprintf(f,"add rule inet parental_control pc_forward ether saddr %s ip daddr %s accept comment \"pc:%s:wl4-out\"\n",m,ip,id);fprintf(f,"add rule inet parental_control pc_forward ether daddr %s ip saddr %s accept comment \"pc:%s:wl4-in\"\n",m,ip,id);}else if(family==AF_INET6){fprintf(f,"add rule inet parental_control pc_forward ether saddr %s ip6 daddr %s accept comment \"pc:%s:wl6-out\"\n",m,ip,id);fprintf(f,"add rule inet parental_control pc_forward ether daddr %s ip6 saddr %s accept comment \"pc:%s:wl6-in\"\n",m,ip,id);}}
static void nft_whitelist_rules(FILE*f,json_object*d,const char*m,const char*id){json_object*entries=NULL;if(!whitelist_active(d)||!json_object_object_get_ex(d,"whitelist_entries",&entries)||!json_object_is_type(entries,json_type_array))return;for(int x=0;x<(int)json_object_array_length(entries);x++){json_object*v=json_object_array_get_idx(entries,x);if(!v||!json_object_is_type(v,json_type_string))continue;const char*entry=json_object_get_string(v);struct in_addr a4;struct in6_addr a6;if(inet_pton(AF_INET,entry,&a4)==1){nft_allow_whitelist_address(f,m,id,AF_INET,&a4);continue;}if(inet_pton(AF_INET6,entry,&a6)==1){nft_allow_whitelist_address(f,m,id,AF_INET6,&a6);continue;}struct addrinfo hints={0},*res=NULL;hints.ai_family=AF_UNSPEC;hints.ai_socktype=SOCK_STREAM;if(getaddrinfo(entry,NULL,&hints,&res)!=0)continue;int emitted=0;for(struct addrinfo*ai=res;ai&&emitted<16;ai=ai->ai_next){if(ai->ai_family==AF_INET){struct sockaddr_in*sa=(struct sockaddr_in*)ai->ai_addr;nft_allow_whitelist_address(f,m,id,AF_INET,&sa->sin_addr);emitted++;}else if(ai->ai_family==AF_INET6){struct sockaddr_in6*sa=(struct sockaddr_in6*)ai->ai_addr;nft_allow_whitelist_address(f,m,id,AF_INET6,&sa->sin6_addr);emitted++;}}freeaddrinfo(res);}}
...
static int nft_commit(void){int rc=nft_apply();if(rc==0){nft_dirty=0;json_object*a=arr();for(int i=0;i<(int)json_object_array_length(a)&&i<128;i++){json_object*d=json_object_array_get_idx(a,i);blocked_cache[i]=is_blocked(d,i);whitelist_cache[i]=whitelist_active(d);}}else{nft_dirty=1;fprintf(stderr,"parental-control: nftables apply failed; will retry\n");}return rc;}
