...
static json_object *arr(void); static json_object *okmsg(const char*); static int sync_dhcp_macs(void); static const char *sval(json_object*,const char*,const char*); static int ival(json_object*,const char*,int); static int bval(json_object*,const char*,int); static int findid(const char*); static int mac_valid(const char*); static void dhcp(json_object*,char*,size_t); static int night(json_object*); static int nft_apply(void); static int nft_commit(void); static void state_device(json_object*,json_object*,int); static json_object *state_snapshot(void);
...
static void runtime_remove(int i){if(i<0||i>=128)return;for(int k=i;k<127;k++){manual[k]=manual[k+1];break_active[k]=break_active[k+1];temporary_unblock[k]=temporary_unblock[k+1];brk_until[k]=brk_until[k+1];brk_until_mono[k]=brk_until_mono[k+1];used_seconds[k]=used_seconds[k+1];session_seconds[k]=session_seconds[k+1];bonus_minutes[k]=bonus_minutes[k+1];previous_inbound[k]=previous_inbound[k+1];previous_outbound[k]=previous_outbound[k+1];counters_seen[k]=counters_seen[k+1];blocked_cache[k]=blocked_cache[k+1];last_active[k]=last_active[k+1];}manual[127]=break_active[127]=temporary_unblock[127]=counters_seen[127]=blocked_cache[127]=0;brk_until[127]=brk_until_mono[127]=last_active[127]=0;used_seconds[127]=session_seconds[127]=bonus_minutes[127]=0;previous_inbound[127]=previous_outbound[127]=0;}
...
static int make_unique_device_id(const char*mac,char out[64]){char base[32];make_device_id(mac,base);if(findid(base)<0){snprintf(out,64,"%s",base);return 0;}for(int n=2;n<1000;n++){snprintf(out,64,"%s-%d",base,n);if(findid(out)<0)return 0;}return-1;}
