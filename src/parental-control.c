...
static void tick(void){static time_t last_dhcp_sync=0;time_t now_sync=monotonic_now();if(now_sync-last_dhcp_sync>=2){sync_dhcp_macs();last_dhcp_sync=now_sync;}
    ...
    for(int i=0;i<(int)json_object_array_length(a)&&i<128;i++){
        ...
        int idle=ival(d,"idle_timeout_seconds",15); if(!before&&!active&&last_active[i]&&idle>0&&mono-last_active[i]<=idle)active=1;
        if(!bval(d,"enabled",1))active=0;
        if(active&&!before){used_seconds[i]+=dt;changed=1;if(cycle&&!break_active[i]){session_seconds[i]+=dt;int lim=ival(d,"session_limit_minutes",0)*60;if(lim>0&&session_seconds[i]>=lim){temporary_unblock[i]=0;start_break_state(i,ival(d,"break_minutes",0)*60);changed=rules_changed=1;}}}
