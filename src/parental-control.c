...
static void tick(void){static time_t last_dhcp_sync=0;time_t now_sync=monotonic_now();if(now_sync-last_dhcp_sync>=2){sync_dhcp_macs();last_dhcp_sync=now_sync;}
    ...
    for(int i=0;i<(int)json_object_array_length(a)&&i<128;i++){
        ...
        if(have){
            if(counters_seen[i]){int reset=inb[i]<previous_inbound[i]||outb[i]<previous_outbound[i];if(reset)fprintf(stderr,"parental-control: nft counter reset device_id=%s in_prev=%llu in_now=%llu out_prev=%llu out_now=%llu nft_dirty=%d pid=%ld\n",sval(d,"id",""),previous_inbound[i],inb[i],previous_outbound[i],outb[i],nft_dirty,(long)getpid());unsigned long long di=inb[i]>=previous_inbound[i]?inb[i]-previous_inbound[i]:inb[i]; unsigned long long do_=outb[i]>=previous_outbound[i]?outb[i]-previous_outbound[i]:outb[i]; unsigned long long threshold=(unsigned long long)ival(d,"activity_threshold_bytes",4096);if(threshold<1)threshold=1; /* A counter reset means nft rules were recreated while this device was passing traffic. Treat non-zero post-reset traffic as activity; otherwise use the per-device threshold. */ if(!before&&((reset&&(di+do_>0))||di+do_>=threshold)){active=1;last_active[i]=mono;}}
