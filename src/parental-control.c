...
static int read_packet_activity(unsigned long long *bytes){memset(bytes,0,sizeof(unsigned long long)*128);if(activity_fd<0)return 0;json_object*a=arr();unsigned char macs[128][6];int valid[128]={0};int n=(int)json_object_array_length(a);if(n>128)n=128;for(int i=0;i<n;i++)valid[i]=mac_bytes(sval(json_object_array_get_idx(a,i),"mac",""),macs[i]);unsigned char buf[65536];int packets=0;for(;packets<8192;packets++){ssize_t len=recv(activity_fd,buf,sizeof buf,MSG_DONTWAIT);if(len<0){if(errno==EAGAIN||errno==EWOULDBLOCK)break;if(errno==EINTR)continue;fprintf(stderr,"parental-control: packet activity recv failed: %s\n",strerror(errno));break;}if(len<14)continue;for(int i=0;i<n;i++)if(valid[i]&&(!memcmp(buf,macs[i],6)||!memcmp(buf+6,macs[i],6))){bytes[i]+=(unsigned long long)len;break;}}return 1;}
