...
static int is_blocked(json_object*d,int i){if(!bval(d,"enabled",1))return 0;int lim=ival(d,"daily_limit_minutes",0)*60+(int)bonus_minutes[i]*60;return manual[i]||(!temporary_unblock[i]&&(night(d)||(break_active[i]&&time(NULL)<brk_until[i])||(bval(d,"daily_limit_enabled",0)&&lim>0&&used_seconds[i]>=lim)));}
